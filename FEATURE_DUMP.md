...
- **F4** (or the ✎ pencil): Edits the selected entry — replaces the palette with the matching dialog (Connect, Mount SFTP, or Mount S3) prefilled with the entry's fields. Saved profiles open in edit mode: the title reads "Edit Connection", the name is prefilled with the "Update connection profile" box pre-checked, and the id stays stable across renames so the profile is updated in place (keychain secrets included — editing an IAM-user S3 profile prefills the key pair from the keychain). Edit mode adds a **Save** button that persists without connecting and returns to the palette; unchecking the update box turns Connect/Mount into a one-off connection that leaves the profile untouched. Recents open the same dialogs plain-prefilled — edit the target, then connect.
