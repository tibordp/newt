...
mod tests {
    ...
    /// of `cargo test`. Run `cargo test --package newt -- --nocapture` to see
    /// the export path; CI can then `git diff --exit-code` to flag a forgotten
    /// regeneration.
    #[test]
    fn export_typescript_bindings() {
        use specta_typescript::{BigIntExportBehavior, Typescript};

        let cfg = Typescript::default()
        ...
        create_specta_builder()
            .export(cfg, "../src/lib/bindings.ts")
