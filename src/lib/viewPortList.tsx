...
const normalizeValue = (min: number, value: number, max = Infinity) =>
  Math.max(Math.min(value, max), min);

const getDiff = (value1: number, value2: number, step: number) =>
...
const ViewportListInner = <T,>(
  ...
  const [[estimatedItemHeight, estimatedItemMargin], setItemDimensions] =
    useState(() => [
    ...
    mainFrameRef.current = () => {
      ...
      if (
        (marginTopRef.current < 0 &&
          topSpacerRect[propName.top] - marginTopRef.current >=
            limitsWithOverscanSize[propName.top]) ||
        (marginTopRef.current > 0 &&
          topSpacerRect[propName.top] >=
            limitsWithOverscanSize[propName.top]) ||
      ...
      if (estimatedItemHeight === 0 || estimatedItemMargin === -1) {
        ...
        findElement({
          ...
          compare: (element) => {
            itemsHeightSum +=
              getItemBoundingClientRect(element)[propName.height];
        ...
        const nextItemHeight =
          estimatedItemHeight === 0
            ? Math.ceil(itemsHeightSum / renderedItemsCount)
            : estimatedItemHeight;
      ...
      if (scrollToIndexOptionsRef.current) {
        ...
        if (targetIndex < startIndex || targetIndex > endIndex) {
          if (scrollToIndexOptionsRef.current.alignToTop) {
            setIndexes([
              targetIndex - 1,
        ...
        const scrollToElementDelay =
          delay < 0 && SHOULD_DELAY_SCROLL ? 30 : delay;
      ...
      const topSecondElement =
        topElement === bottomSpacer
          ? bottomSpacer
          : (topElement.nextSibling as Element);
      const bottomSecondElement =
        bottomElement === topSpacer
          ? topSpacer
      ...
      const isAllBelowBottom =
        bottomSpacerRect[propName.top] < limitsWithOverscanSize[propName.top];
      const isTopBelowTop =
        !isAllAboveTop &&
        !isAllBelowBottom &&
      ...
      const isBottomSecondAboveTop =
        ...
        (bottomSecondElement === topSpacer
          ? topSpacerRect
          : getItemBoundingClientRect(bottomSecondElement))[propName.bottom] >
          limitsWithOverscanSize[propName.bottom];
      const isTopSecondAboveTop =
        ...
        (topSecondElement === bottomSpacer
          ? bottomSpacerRect
          : getItemBoundingClientRect(topSecondElement))[propName.top] <
          limitsWithOverscanSize[propName.top];
      ...
      if (isBottomSecondAboveTop) {
        const [, index] = findElement({
          ...
          asc: false,
          compare: (element) =>
            getItemBoundingClientRect(element)[propName.bottom] <=
            limitsWithOverscanSize[propName.bottom],
        });
      ...
      if (isTopSecondAboveTop) {
        const [, index] = findElement({
          ...
          fromIndex: startIndex,
          compare: (element) =>
            getItemBoundingClientRect(element)[propName.top] >=
            limitsWithOverscanSize[propName.top],
        });
  ...
  if (anchorElementRef.current && getViewport() && topSpacerRef.current) {
    anchorHeightOnRender =
      ...
      (getViewport() === document.documentElement
        ? 0
        : getViewport().getBoundingClientRect()[propName.top]);
  }
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const offset =
      ...
      (viewport === document.documentElement
        ? 0
        : viewport.getBoundingClientRect()[propName.top]) -
      anchorHeightOnRender;
